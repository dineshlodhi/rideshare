...
if(isset($_POST['submit'])){
  ...
  $vehicletyp = $_POST['vehicletype'][0];
  ...
  if($output1->status == 'OK' and $output2->status == 'OK') 
    { // Check if address is available or not
      ...
      //check for trips, show. if not add trip
      ...
      <?php
        ...
        $_SESSION['vehicletyp']=$vehicletyp;
            ...
            while ($trip = mysql_fetch_array($curtrips))
                {
                  ...
                  $destdist = distanceCalculation($trip['dlat'],$trip['dlong'],$lat2,$long2);
                  $tdif = abs( (strtotime($trip['time'])-strtotime($time))/60);
                  if ($sourcedist<3 && $destdist<3 && $tdif<60 && $date == $trip['date'])
                    {
                      $flag=1;
                      echo '<div class="panel panel-default">  <div class="panel-body">
                          <dl class="dl-horizontal">
      ...
      else
      {
        ...
        <p>If the above similar trips did not suit you, proceed to add trip</p>
